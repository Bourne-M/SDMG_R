...
class SDMGRHead(nn.Module):
    ...
    def forward(self, relations, texts, x=None):
        ...
        valid = torch.where(all_nums > 0)[0]
        nodes[valid] = rnn_nodes[valid].gather(
            1, (all_nums[valid] - 1).unsqueeze(-1).unsqueeze(-1).expand(-1, -1, rnn_nodes.size(-1))).squeeze(1)

        if x is not None:
            nodes = self.fusion([x, nodes])

        all_edges = torch.cat(
            [rel.contiguous().view(-1, rel.size(-1)) for rel in relations])
